...
contract THEFIGURE is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "TFDao";
    string private constant _name = "The Figure";

    uint256 public TFDao_max_BuyLimit = 20000000000 * 10**9;
    uint256 public TFDao_max_SellLimit = 5000000000 * 10**9;

    address public TFDao_Figure_Wallet = 0x52e54Af5ed9EDA98C6f7c0E66B928b4E392c6259;
    address private TFDao_Investment_Wallet = 0xf0f37d932d4C7fD1f6f0edBfA034DB6576Fd74D5;

    uint256 public TFDao_max_WalletLimit = 20000000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 6, 2, 4);
    Taxes public taxes = Taxes(0, 6, 2, 4);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[TFDao_Figure_Wallet] = true;
        _isExcludedFromFee[TFDao_Investment_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[TFDao_Figure_Wallet] = true;
        allowedTransfer[TFDao_Investment_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= TFDao_max_BuyLimit, "You are exceeding TFDao_max_BuyLimit");
            require(balanceOf(to) + amount <= TFDao_max_WalletLimit, "You are exceeding TFDao_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= TFDao_max_SellLimit, "You are exceeding TFDao_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= TFDao_max_WalletLimit, "You are exceeding TFDao_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(TFDao_Figure_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(TFDao_Investment_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function TFDao_Update_MarketingWallet(address newWallet) external onlyOwner{
        TFDao_Figure_Wallet = newWallet;
    }
    
    function TFDao_Update_TeamWallet(address newWallet) external onlyOwner{
        TFDao_Investment_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        TFDao_max_BuyLimit = maxBuy * 10**decimals();
        TFDao_max_SellLimit = maxSell * 10**decimals();
    }
    
    function TFDao_Max_Walletlimit(uint256 amount) external onlyOwner{
        TFDao_max_WalletLimit = amount * 10**decimals();
    }
